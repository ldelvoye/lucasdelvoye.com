...
export function introScript(version: string): Step[] {
  const tarball = `https://github.com/ldelvoye/smorg/releases/download/v${version}/smorg-${version}.tar.gz`;
  const cellar = `/opt/homebrew/Cellar/smorg/${version}`;
  return [
